...
const webpackConfig: webpack.Configuration = {
    ...
    plugins: [
        ...
        new AssetsWebpackPlugin({
            ...
            processOutput: function (assets: Record<string, Record<string, string | string[]>>) {
                const stripAuto = (s: string) => s.replace('auto/', '');
                const finalAssets: Record<string, Record<string, string | string[]>> = {};
                for (const a in assets) {
                    if (a.length > 0) {
                        for (const b in assets[a]) {
                            const value = assets[a][b];
                            assets[a][b] = Array.isArray(value)
                                ? value.map(stripAuto)
                                : stripAuto(value);
                        }
