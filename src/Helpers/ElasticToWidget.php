...
class ElasticToWidget
{
    ...
    private static ?array $shemaMap = null;
    private static function getSchemaMap(?int $id) {
        if (static::$shemaMap === null) {
            static::$shemaMap = [];
            foreach(ElasticSchema::query()->each() as $schema) {
                static::$shemaMap[$schema->getId()] = strtolower($schema->getName());
            }
        }
        $name = null;
        if ($id !== null) {
            $name = isset(static::$shemaMap[$id]) ? static::$shemaMap[$id] : null;
        }
        return $name;
    }
    ...
    {
        $widget = null;
        $schemaName = self::getSchemaMap($elastic->getElasticSchemaId());
    ...
    {
        $result = null;
        $schemaName = self::getSchemaMap($items[0]->getElasticSchemaId());
    ...
    public static function convertAll($blocsquery): array
    {
        ...
        foreach ($merged as $item) {
            if (is_array($item) === true) {
                $schemaName = self::getSchemaMap($item[0]->getElasticSchemaId());
                $header = null;
